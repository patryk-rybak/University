...
SELECT offer.title,
       ...
       (array_agg(DISTINCT skill.name))[1:4] as dupa
FROM offer
JOIN skill ON (offer.id = skill.offer_id)
...
SELECT dupcia.city, COUNT(cyce.id)
FROM dupcia
LEFT JOIN cyce ON (dupcia.city = cyce.city)
GROUP BY (dupcia.city)
ORDER BY COUNT(cyce.id) DESC;
...
WITH
    ...
    cyce AS (
        ...
        FROM nation
        JOIN dupa ON (n_nationkey = c_nationkey)
        GROUP BY n_nationkey
    )
...
LEFT JOIN (
    ...
    FROM dupa
    JOIN cyce ON (dupa.c_nationkey = cyce.n_nationkey)
    WHERE (sum = max)
