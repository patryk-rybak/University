...
class Player(object):
    ...
    def manhattan_distance_and_animal_weight(self):
        summary_dist = 0
        summary_weight = 0
        dest_den = self.game.dens[self.my_player]
        
        for piece in self.game.pieces[self.my_player]:
            summary_weight += piece
            pos = self.game.pieces[self.my_player][piece]
            summary_weight += abs(pos[0] - dest_den[0]) + abs(pos[1] - dest_den[1])
                
        return summary_dist, summary_weight
    
    def heuristic():
        pass
    
    def find_best(self, moves):
        pass
    ...
    def loop(self):
        while True:
            ...
            moves = list(filter(lambda x: abs(x[0][0] - self.game.dens[1 - self.my_player][0]) + abs(x[0][1] - self.game.dens[1 - self.my_player][1]) > abs(x[1][0] - self.game.dens[1 - self.my_player][0]) + abs(x[1][1] - self.game.dens[1 - self.my_player][1]), moves))
            if moves:
                # move = random.choice(moves)
                move = find_best(moves)
                self.game.do_move(move)
